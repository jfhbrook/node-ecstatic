...
module.exports = (opts) => {
  ...
  const selectDisplay = opts.selectDisplay;
      ...
      fs.readdir(dir, (readErr, _files) => {
        ...
        function render(dirs, renderFiles, lolwuts) {
          ...
          const writeRow = (file) => {
            ...
            html += '<tr>';
            const selectDisplayOptions = selectDisplay.split('|');
            for (let i = 0; i < selectDisplayOptions.length; i += 1) {
              switch (selectDisplayOptions[i].trim()) {
                case 'icon':
                  html += `${'<td><i class="icon '}${iconClass}"></i></td>`;
                  break;
                case 'permissions':
