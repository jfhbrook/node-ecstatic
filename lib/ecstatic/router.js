...
function isCompress(el) {
  return ['*','compress', 'gzip', 'deflate'].indexOf(el) != -1;
};

function shouldCompress(req) {
  var headers = req.headers;
  return headers && headers['accept-encoding'] && headers['accept-encoding'].split(",").some(isCompress);
};
...
module.exports = function router (opts, cb) {
  ...
  if(shouldCompress(req)) {
    ...
    fs.stat(gzippedFile, function (err, stat) {
      if(!err && stat.isFile()) return routeFile(gzippedFile, opts, cb);

      return routeFile(file, opts, cb)
    });
  } else {
    return routeFile(file, opts, cb);
  }
...
function routeFile(file, opts, cb) {
  var req = opts.req;
