      ...
      fs.readdir(dir, function (err, files) {
        ...
        function render(dirs, files, lolwuts) {
          ...
          var html = [
            ...
            '    <link href="/_ecstatic-assets/ecstatic.css" rel="stylesheet" type="text/css" />',
            (icons ? '    <link href="/_ecstatic-assets/ecstatic-icons.css" rel="stylesheet" type="text/css" />' : ''),
          ...
          html += '<br><address>Node.js ' +
            ...
            (icons ? '<a href="http://www.famfamfam.com/lab/icons/silk/">Silk Icons</a>':'')
            +
            '</body></html>'
          ;
...
// create CSS class from a file extension
function createCssClass(displayName){
  
  var s = path.extname(displayName).replace('.','').toLowerCase();;

  // CSS classes must start with a-z or - or _
  if ( !/[a-z\_\-]/.test(s.charAt(0)) ) {
    s = '_' + s;
  }
