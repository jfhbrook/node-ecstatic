    ...
    */
    ...
    if (opts.gzip && shouldCompress(req)) {
      fs.stat(gzipped, function (err, stat) {
        if (!err && stat.isFile()) {
          hasGzipId12(gzipped, function (isGzip) {
            if (isGzip) {
              file = gzipped;
              return serve(stat);
            } else {
...
function hasGzipId12(gzipped, cb) {
  var stream = fs.createReadStream(gzipped, { start: 0, end: 1, autoClose: true }),
      result = false;
