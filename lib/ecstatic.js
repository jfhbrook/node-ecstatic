...
var ecstatic = module.exports = function (dir, options) {
  ...
  return function middleware (req, res, next) {
    ...
    // try and serve assets
    var assetsDirectory = '_ecstatic-assets'
    
    if(opts.showDir && path.normalize(pathname).slice(1,assetsDirectory.length+1) === assetsDirectory){
      file = path.join(__dirname,pathname);
    }
    else{
