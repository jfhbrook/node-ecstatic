...
function getNscaDateTime() {
  ...
  function sign(number) {
    let prefix = '';
    if (number < 0) prefix = '-';
    else if (number > 0) prefix = '+';
    return prefix + ((number < 10 && '0') + Math.abs(number));
  }
  ...
  const timezone = `${sign(now.getTimezoneOffset() / 60)}00`; // TODO: minutes offset
  return `${date}/${month}/${year}:${hour}:${minute}:${seconds} ${timezone}`;
}
...
module.exports = function createMiddleware(_dir, _options) {
  ...
  return function middleware(req, res, next) {
    ...
    */
    ...
    let is400 = false; // TODO: refactor all status code to provide correct code to logger
    try {
      ...
      if (logging) {
        ...
        console.log(`${req.headers.host} - - [${getNscaDateTime()}] "${req.method} ${req.url}" ${is400 ? 400 : 200} -`); // TODO: don't have bytes before fs.stat
        // log format: host rfc931 username date:time timezone request statuscode bytes
        // - is used if don't have the information
        // date:time timezone = [dd/MMM/yyyy:hh:mm:ss +-hhmm]
      }
    }
