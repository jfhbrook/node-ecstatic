...
var ecstatic = module.exports = function ecstatic (dir, options) {
  ...
  return function middleware (req, res, next) {
    ...
    if (file.slice(0, root.length) !== root) {
      return status[403](req, res, next);
    }

    // Only 'get' and 'head' are allowed.
    if (req.method && (req.method !== 'GET' && req.method !== 'HEAD' )) {
      return status[405](req, res, next);
    }
    ...
    fs.stat(file, function (err, stat) {
      ...
      if (err && err.code === 'ENOENT') {
        status[404](req, res, next);
      }
      // Other types of errors are not expected.
      else if (err) {
        status[500](req, res, next, { error: err });
      }
        ...
        if (autoIndex) {
          ...
          fs.stat(path.resolve(file, 'index.html'), function (err, stat) {
            if ((err && err.code === 'ENOENT') || stat.isDirectory()) {
              ...
              // serve up that index.html
              // TODO: Make serve a middleware
              serve(req, res, next, stat);
            }
          });
        }
        else {
